-- LOCUS Journey Studio initial schema with RLS
...
create or replace function public.is_assigned_orchestrator(p_cohort_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where cm.cohort_id = p_cohort_id
      and cm.assigned_orchestrator_id = auth.uid()
  );
$$;
...
create or replace function public.journey_version_org_id(p_version_id uuid)
returns uuid
language sql
stable
security definer
...
create or replace function public.journey_org_id(p_journey_id uuid)
returns uuid
language sql
stable
security definer
...
create or replace function public.journey_version_is_editable(p_version_id uuid)
returns boolean
language sql
stable
security definer
set search_path = public
as $$
  select jv.status in ('concept', 'draft', 'field_test', 'private_adult_walk', 'learner_pilot')
  from public.journey_versions jv
  where jv.id = p_version_id;
$$;
...
-- learner_profiles
create policy "learners_select_org_member" on public.learner_profiles for select to authenticated
  using (public.is_org_member(organization_id));
create policy "learners_insert_org_staff" on public.learner_profiles for insert to authenticated
  with check (public.has_org_role(organization_id, array['owner', 'admin', 'creator', 'orchestrator']::public.org_role[]));
create policy "learners_update_org_staff" on public.learner_profiles for update to authenticated
  using (public.has_org_role(organization_id, array['owner', 'admin', 'creator', 'orchestrator']::public.org_role[]));
...
create policy "journeys_insert_creator" on public.journeys for insert to authenticated
  with check (public.has_org_role(organization_id, array['owner', 'admin', 'creator']::public.org_role[]));
create policy "journeys_update_creator" on public.journeys for update to authenticated
  using (public.has_org_role(organization_id, array['owner', 'admin', 'creator']::public.org_role[]));

-- journey_versions
create policy "journey_versions_select_member" on public.journey_versions for select to authenticated
  using (public.is_org_member(public.journey_version_org_id(id)));
create policy "journey_versions_insert_creator" on public.journey_versions for insert to authenticated
  with check (public.has_org_role(public.journey_org_id(journey_id), array['owner', 'admin', 'creator']::public.org_role[]));
create policy "journey_versions_update_editable" on public.journey_versions for update to authenticated
  using (
    public.has_org_role(public.journey_version_org_id(id), array['owner', 'admin', 'creator']::public.org_role[])
    and public.journey_version_is_editable(id)
  );

-- journey_stops
create policy "journey_stops_select_member" on public.journey_stops for select to authenticated
  using (public.is_org_member(public.journey_version_org_id(journey_version_id)));
create policy "journey_stops_mutate_editable" on public.journey_stops for all to authenticated
  using (
    public.has_org_role(public.journey_version_org_id(journey_version_id), array['owner', 'admin', 'creator']::public.org_role[])
    and public.journey_version_is_editable(journey_version_id)
  )
  with check (
    public.has_org_role(public.journey_version_org_id(journey_version_id), array['owner', 'admin', 'creator']::public.org_role[])
    and public.journey_version_is_editable(journey_version_id)
...
create policy "branches_mutate_editable" on public.adaptive_branches for all to authenticated
  using (
    ...
    and public.journey_version_is_editable(
      (select js.journey_version_id from public.journey_stops js where js.id = journey_stop_id)
    )
  )
  with check (
...
create policy "cohorts_select_member" on public.cohorts for select to authenticated
  using (public.is_org_member(organization_id));
create policy "cohorts_mutate_staff" on public.cohorts for all to authenticated
  using (public.has_org_role(organization_id, array['owner', 'admin', 'orchestrator', 'creator']::public.org_role[]))
  with check (public.has_org_role(organization_id, array['owner', 'admin', 'orchestrator', 'creator']::public.org_role[]));

-- cohort_memberships
create policy "cohort_memberships_select" on public.cohort_memberships for select to authenticated
  using (
    public.is_org_member((select c.organization_id from public.cohorts c where c.id = cohort_id))
    or assigned_orchestrator_id = auth.uid()
  );
create policy "cohort_memberships_mutate_staff" on public.cohort_memberships for all to authenticated
  using (
    public.has_org_role(
      (select c.organization_id from public.cohorts c where c.id = cohort_id),
      array['owner', 'admin', 'orchestrator']::public.org_role[]
    )
  )
  with check (
    public.has_org_role(
...
-- journey_enrollments
create policy "enrollments_select" on public.journey_enrollments for select to authenticated
  using (
    public.is_org_member((select c.organization_id from public.cohorts c where c.id = cohort_id))
    or public.is_assigned_orchestrator(cohort_id)
  );
create policy "enrollments_mutate_staff" on public.journey_enrollments for all to authenticated
  using (
    public.has_org_role(
      (select c.organization_id from public.cohorts c where c.id = cohort_id),
      array['owner', 'admin', 'orchestrator']::public.org_role[]
...
create policy "field_notes_select" on public.field_notes for select to authenticated
  using (
    public.is_org_member(public.enrollment_org_id(journey_enrollment_id))
    and (
      visibility <> 'private'
      or created_by_profile_id = auth.uid()
      or public.is_assigned_orchestrator((select je.cohort_id from public.journey_enrollments je where je.id = journey_enrollment_id))
    )
  );
create policy "field_notes_insert" on public.field_notes for insert to authenticated
  with check (
    public.is_org_member(public.enrollment_org_id(journey_enrollment_id))
    and (
      public.has_org_role(public.enrollment_org_id(journey_enrollment_id), array['owner', 'admin', 'orchestrator', 'creator']::public.org_role[])
      or created_by_profile_id = auth.uid()
    )
  );
...
create policy "interventions_mutate_staff" on public.mentor_interventions for all to authenticated
  using (
    public.has_org_role(
      public.enrollment_org_id(journey_enrollment_id),
      array['owner', 'admin', 'orchestrator', 'creator']::public.org_role[]
    )
    or public.is_assigned_orchestrator((select je.cohort_id from public.journey_enrollments je where je.id = journey_enrollment_id))
  )
  with check (
...
create policy "artifacts_select" on public.artifacts for select to authenticated
  using (public.is_org_member(public.enrollment_org_id(journey_enrollment_id)));
create policy "artifacts_mutate_staff" on public.artifacts for all to authenticated
  using (
    public.has_org_role(
      public.enrollment_org_id(journey_enrollment_id),
      array['owner', 'admin', 'orchestrator', 'creator']::public.org_role[]
...
create policy "resurfacing_mutate" on public.resurfacing_events for all to authenticated
  using (public.is_org_member(public.enrollment_org_id(journey_enrollment_id)))
  with check (public.is_org_member(public.enrollment_org_id(journey_enrollment_id)));

-- journey_reviews (reviewers write reviews, not journey content)
...
create policy "reviews_insert_reviewer" on public.journey_reviews for insert to authenticated
  with check (
    public.has_org_role(public.journey_version_org_id(journey_version_id), array['owner', 'admin', 'reviewer']::public.org_role[])
...
create policy "reviews_update_reviewer" on public.journey_reviews for update to authenticated
  using (
    ...
    and public.has_org_role(public.journey_version_org_id(journey_version_id), array['owner', 'admin', 'reviewer']::public.org_role[])
  );
...
create policy "media_select" on public.media_assets for select to authenticated
  using (
    ...
    and (owner_profile_id = auth.uid() or visibility <> 'private')
  );
create policy "media_insert" on public.media_assets for insert to authenticated
  with check (public.is_org_member(organization_id) and owner_profile_id = auth.uid());
create policy "media_delete_own" on public.media_assets for delete to authenticated
...
create policy "audit_insert_member" on public.audit_events for insert to authenticated
  with check (public.is_org_member(organization_id) and actor_profile_id = auth.uid());
...
create or replace function public.storage_org_from_path(path text)
returns uuid
language sql
immutable
as $$
...
create policy "field_media_insert" on storage.objects for insert to authenticated
  with check (
    ...
    and public.is_org_member(public.storage_org_from_path(name))
...
create policy "field_media_select" on storage.objects for select to authenticated
  using (
    ...
    and (
      ...
      or public.has_org_role(public.storage_org_from_path(name), array['owner', 'admin', 'orchestrator']::public.org_role[])
    )
  );
